...
export class UserService {
  ...
  public readonly loggedInUserObs: Observable<User> =
    this.loggedInUserSub.asObservable();

  private showVerifyOTPSub = new BehaviorSubject<{
    requestOTP: boolean;
    otpMailId: string;
  }>({ requestOTP: false, otpMailId: null });
  public readonly showVerifyOTPObs: Observable<{
    requestOTP: boolean;
    otpMailId: string;
  }> = this.showVerifyOTPSub.asObservable();
  ...
  register(user: User): Observable<any> {
    ...
    return this.apiService.post(this.constructUrl('register'), user).pipe(
      map((user) => {
        if (user.status == 1) {
  ...
  verifyOTP(otpData: { otp: string; email: string }) {
    return this.apiService.post(this.constructUrl('verify-otp'), otpData).pipe(
      map((resp) => {
        if (resp.status == 1) {
  ...
  resendOTP() {
    return this.apiService
      .post(this.constructUrl('resend-verify-otp'), {
  ...
  login(user: User): Observable<any> {
    ...
    return this.apiService.post(this.constructUrl('login'), user).pipe(
      map((userResp) => {
  ...
  getPasswordChangeOTP(email: string): Observable<any> {
    return this.apiService
      .post(this.constructUrl('change-pass-otp'), { email: email })
      .pipe(
        map((resp) => {
          if (resp.status == 1) {
            //this.login(this.loggedInUser).subscribe();
  ...
  changePasswordByOTP(
    ...
    return this.apiService
      .post(this.constructUrl('change-pass-by-otp'), {
        email: email,
        otp: otp,
        newPassword: newPass,
      })
      .pipe(
        map((resp) => {
          if (resp.status == 1) {
            this.notify.mesg =
              'Your password has been changed successfully! Please use the new password to login.';
            this.notify.errorEvent = NoteEvent.Server;
            this.notifyServ.showError(this.notify);
            this.showVerifyOTPSub.next({ requestOTP: false, otpMailId: null });
            return true;
  ...
  updateProfile(user: User): Observable<any> {
    return this.apiService
      .post(this.constructUserUrl('update-profile'), user)
      .pipe(
        map((resp) => {
          if (resp.status == 1) {
  ...
  updateAddress(addr: UserAddress) {
    return this.apiService
      .post(this.constructUserUrl('update-address'), {
        email: this.loggedInUser.email,
        ...addr,
      })
      .pipe(
        map((resp) => {
          if (resp.status == 1) {
            this.notify.mesg =
              'Your deliver address has been updated successfully!';
            this.notify.errorEvent = NoteEvent.Server;
            this.notifyServ.showError(this.notify);
